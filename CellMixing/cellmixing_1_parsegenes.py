# Changed to count ALL genes using gene_id. 
# NL
import os
import glob
import csv
...
# This script is meant to be run on the .bam files 
# for the mouse human mixing experimens (436, 438).

# Set the folder for the .bam files that have been deduped from UMI tools.
# Files should have *dedup.bam. Script also uses pysam which requires an 
...
BAMFILES = sorted(glob.glob('%s*Merged_Dedup.bam' % PATH_BAMFILES))
...
class Parser(object):
    ...
    def setDF(self):
        ...
        # Finds first instance of a mouse gene
        m_index = list(map(
            lambda name: "ENSMUSG" in name, list(df.index))
            ).index(True)
...
def main():
    for bamfile in BAMFILES:
        ...
        HUMAN_READS, MOUSE_READS = parseReads(
            gindex_human, gindex_mouse, samfile)
        ...
        #   PATH_GENES 
        #   + HUMAN_MAPS.fname_short + HUMAN_MAPS.species + "bc1_AllCounts.csv")
