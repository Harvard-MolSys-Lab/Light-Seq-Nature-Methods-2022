# NOTE: This file overwrites previous .featurecounts.bam files, so should only
# be used last in the pipeline.
...
def deduplicate(filePrefix, geneFile, flags=''):
  ...
  os.system('featureCounts -a %s %s -o %s -R BAM %s' % \
            (GFF_FILE, flags, assignedGeneFile, geneFile))
...
def main():  
  ...
  for geneFile in GENE_FILES:
    ...
    deduplicate('%sExon' % (filePrefix), geneFile)
    deduplicate('%sGeneBody' % (filePrefix), geneFile, flags='-t gene')

    exonDedupGeneFile = '%sExon_Dedup.bam' % filePrefix
    os.system('samtools index %s' % exonDedupGeneFile)

    geneBodyDedupGeneFile = '%sGeneBody_Dedup.bam' % filePrefix
    os.system('samtools index %s' % geneBodyDedupGeneFile)

    allMaps = defaultdict(lambda: [[], []])

    for i, dedupFile in enumerate([exonDedupGeneFile, geneBodyDedupGeneFile]):

      samFile = pysam.AlignmentFile(dedupFile, "rb")
      samIter = samFile.fetch(until_eof=True)

      for read in samIter:
        geneName = read.get_tag('XT')
        allMaps[geneName][i].append(read.query_name)

    diffs = []
    
    for gene in allMaps:
      diff = len(allMaps[gene][1]) - len(allMaps[gene][0])
      diffs.append(diff)

    plt.figure()
    plt.hist(diffs, bins=100)
    plt.yscale('log')
    plt.title(filePrefix.split('/')[1])
    plt.xlabel('Gene - Exon counts')
    plt.ylabel('Number of genes')
    plt.savefig('%s%s_IntronDiff.svg' % (PLOT_DIR, filePrefix.split('/')[1]))
    plt.close()
