...
# Load Data 
countData <- read.csv('./Reordered_Retina_Counts.csv', header = TRUE, sep = ",")
head(countData)
mydata2 = select(countData, -1)
Counts=aggregate(mydata2[,-1],mydata2["Gene.name"],sum)
row.names(Counts) <- Counts$Gene.name
Counts[1] <- NULL

# Import metadata array
metaData <- read.csv('./RetinaMetaData.csv', header = TRUE, sep = ",")
...
results.RGC.BP <- results(DESeq.ds,  pAdjustMethod="BH", contrast=c("condition","BP","GCL"))
...
results.ONL.RGC <- results(DESeq.ds,  pAdjustMethod="BH", contrast=c("condition","GCL","ONL"))
